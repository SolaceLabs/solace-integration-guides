...
This document demonstrates how to integrate Solace Java Message Service (JMS) with the Jakarta era versions of the JBoss Application Server (v8 and up) and Wildfly (v27 and up) for production and consumption of JMS messages. The goal of this document is to outline best practices for this integration to enable efficient use of both the application server and Solace JMS. 
...
Note: this document provides instructions on configuring and deploying the Solace JCA 2.0 resource adapter in JBoss EAP v8+ and Wildfly v27+.  For detailed background on either Solace JMS or the JBoss Application Server refer to the referenced documents below.
...
The Solace JCA 2.0 resource adapter is provided as a standalone RAR file and is versioned together with a specific release of the Solace JMS API.  The JMS API libraries are bundled inside a single resource adapter RAR file for deployment to the JBoss Application Server.

| **Resource** | **File Location** |
| Solace JCA 2.0 resource adapter stand-alone RAR file (Jakarta variant) | <a href="http://dev.solace.com/downloads/download_jms-api/" target="_top">http://dev.solace.com/downloads/download_jms-api/</a> or <br/> https://sftp.solacesystems.com/~[customer]/[version]/Topic_Routing/APIs/JMS/Current/[release]/sol-jms-ra-jakarta-[release].rar |
...
The following steps will make the Solace resource adapter available to all enterprise applications (Refer to the above section [Description of Resources Required](description-of-resources-required) for the file location of the Solace JCA 2.0 Resource Adapter RAR file). 
...
Step 2 - Copy the Solace JCA 2.0 Resource Adapter RAR file to the module "main" directory and unzip the contents of the RAR file.  Example contents (where "xx" is the software version packaged with the specific RAR file):

```
-rw-r--r--. 1 user group   61829 Feb 23 19:36 commons-logging-xx.jar
-rw-r--r--. 1 user group   58964 Feb 23 19:36 jakarta.jms-api-xx.jar
-rw-r--r--. 1 user group   20860 Feb 18 11:43 licenses.txt
drwxr-xr-x. 3 user group     100 Feb 23 19:36 META-INF
-rw-r--r--. 1 user group  365821 Feb 23 19:36 netty-buffer-xx.jar
-rw-r--r--. 1 user group  153180 Feb 23 19:36 netty-codec-base-xx.jar
-rw-r--r--. 1 user group  182890 Feb 23 19:36 netty-codec-compression-xx.jar
-rw-r--r--. 1 user group  684149 Feb 23 19:36 netty-codec-http-xx.jar
-rw-r--r--. 1 user group  132168 Feb 23 19:36 netty-codec-socks-xx.jar
-rw-r--r--. 1 user group  793123 Feb 23 19:36 netty-common-xx.jar
-rw-r--r--. 1 user group  582130 Feb 23 19:36 netty-handler-xx.jar
-rw-r--r--. 1 user group   27163 Feb 23 19:36 netty-handler-proxy-xx.jar
-rw-r--r--. 1 user group   38458 Feb 23 19:36 netty-resolver-xx.jar
-rw-r--r--. 1 user group  563344 Feb 23 19:36 netty-transport-xx.jar
-rw-r--r--. 1 user group  156973 Feb 23 19:36 netty-transport-classes-epoll-xx.jar
-rw-r--r--. 1 user group   42006 Feb 23 19:36 netty-transport-native-epoll-xx.jar
-rw-r--r--. 1 user group   46864 Feb 23 19:36 netty-transport-native-unix-common-xx.jar
-rw-r--r--. 1 user group   78835 Feb 23 19:36 org.apache.servicemix.bundles.jzlib-xx.jar
-rw-r--r--. 1 user group   13199 Feb 23 19:36 org.osgi.annotation-xx.jar
-rw-r--r--. 1 user group   14902 Feb 23 19:36 solace-serdes-xx.jar
-rw-r--r--. 1 user group    3133 Feb 23 19:36 sol-jms-compat-xx.jar
-rw-r--r--. 1 user group 1953867 Feb 23 19:36 sol-jms-jakarta-xx.jar
-rw-r--r--. 1 user group  175288 Feb 23 19:36 sol-jms-jakarta-ra-xx.jar
```

Step 3 - In the module "main" directory, create a "module.xml" file that references the JAR libraries in the Solace JCA 2.0 Resource Adapter and specifies other external dependencies.  Example (update the string "xx" with the JAR versions included in the Solace JCA 2.0 Resource Adapter archive):
...
<module xmlns="urn:jboss:module:1.1" name="com.solacesystems.ra" >
    <resources>
        <resource-root path="." />
        <resource-root path="commons-logging-xx.jar" />
        <resource-root path="netty-buffer-xx.jar" />
        <resource-root path="netty-codec-base-xx.jar" />
        <resource-root path="netty-codec-compression-xx.jar" />
        <resource-root path="netty-codec-http-xx.jar" />
        <resource-root path="netty-codec-socks-xx.jar" />
        <resource-root path="netty-common-xx.jar" />
        <resource-root path="netty-handler-xx.jar" />
        <resource-root path="netty-handler-proxy-xx.jar" />
        <resource-root path="netty-resolver-xx.jar" />
        <resource-root path="netty-transport-xx.jar" />
        <resource-root path="netty-transport-classes-epoll-xx.jar" />
        <resource-root path="netty-transport-native-epoll-xx.jar" />
        <resource-root path="netty-transport-native-unix-common-xx.jar" />
        <resource-root path="org.apache.servicemix.bundles.jzlib-xx.jar" />
        <resource-root path="org.osgi.annotation-xx.jar" />
        <resource-root path="solace-serdes-xx.jar" />
        <resource-root path="sol-jms-jakarta-xx.jar" />
        <resource-root path="sol-jms-compat-xx.jar"/>
        <resource-root path="sol-jms-jakarta-ra-xx.jar"/>
    </resources>
...
:
    <dependencies>
        ...
        <module name="com.solacesystems.ra" export="true" />
    </dependencies>
:
```
...
```
    ...
    <system-properties>
        <property name="soljmsra.classLoaderOverride" value="true"/>
    </system-properties>
    <management>
        [...]
...
...
| ConnectionURL | tcp://__IP:Port__ | Update the value '__IP:Port__' with the actual event broker message-backbone VRF IP |
...
| ExtendedProps |  | Semicolon-separated list for [advanced control of the connection]({{ site.links-docs-jms-properties }}){:target="_top"}.  Not show in the above example. Supported values are shown below. |
...
```xml
    <config-property name="ExtendedProps">solace_JMS_CompressionLevel=9</config-property>
```
...
| **Name** | **Value** | **Description** |
| ConnectionFactoryJndiName | JNDI/Sol/CF | The JNDI name of the JMS connection factory as configured on the Solace event broker. |
...
1. For the "connectionFactoryJndiName" property, specify the value "JNDI/Sol/CF" (this is the value configured on the Solace event broker in section [Setting up Solace JNDI References](#setting-up-solace-jndi-references)).
1. For the "destination" property, specify the value "JNDI/Sol/Q/requests" (the value configured on the Solace event broker in section [Setting up Solace JNDI References](#setting-up-solace-jndi-references).
1. For the "destinationType" property, specify the value "jakarta.jms.Queue". 
...
| destinationType | jakarta.jms.Queue | The JMS class name for the desired destination type. |
...
1. Add Jakarta EE API jar library to the project build path. This can be your application server's Jakarta EE library or download and include a general one, such as from [org.apache.openejb » javaee-api](https://mvnrepository.com/artifact/org.apache.openejb/javaee-api ).
...
#### Enabling XA Support for JMS Connection Factories – Solace Event Broker<a name="enabling-xa-support"></a>
...
* jakarta.resource.ResourceException or one of its subclasses
